...
public class AdminsDao {
    ...
    public Admins create(Admins admins) {
        try (Connection conn = DbUtil.getConnection()) {
            PreparedStatement statement = conn.prepareStatement(CREATE_ADMINS_QUERY, Statement.RETURN_GENERATED_KEYS);
    ...
    public Admins[] findAll() {
        try (Connection connection = DbUtil.getConnection()) {
            ...
            while (resultSet.next()) {
                ...
                admins.setEnable(resultSet.getInt("enable"));
                admins.setId(resultSet.getInt("id"));
                arrayOfAdmins = addToArray(admins,arrayOfAdmins);
            }
    ...
    private Admins[] addToArray(Admins admins, Admins[] arrayOfAdmins) {
        Admins[] tmpAdmins = Arrays.copyOf(arrayOfAdmins, arrayOfAdmins.length + 1);
        tmpAdmins[arrayOfAdmins.length] = admins;
        return tmpAdmins;
    }
